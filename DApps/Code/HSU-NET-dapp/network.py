...
class HSU_Net(nn.Module):
    def __init__(self, img1_ch=1,img2_ch=5, output_ch=5, t=2):
        ...
        self.RRCNN1 = RRCNN_block(ch_in=img1_ch, ch_out=16, t=t) #64
        self.RRCNN1_2 = RRCNN_block(ch_in=img2_ch, ch_out=32, t=t) #64
        self.RRCNN2 = RRCNN_block(ch_in=32, ch_out=64, t=t) #64 128

        self.RRCNN3 = RRCNN_block(ch_in=64, ch_out=128, t=t) #256 256

        # self.Up3 = up_conv(ch_in=256, ch_out=128) #256 128
        self.Att3 = Attention_block(F_g=64, F_l=64, F_int=32) #128 128 64
        self.Up_RRCNN3 = RRCNN_block(ch_in=128, ch_out=64, t=t) #256 128

        self.Up2 = up_conv(ch_in=64, ch_out=32) #128 64

        self.Up3 = up_conv(ch_in=128, ch_out=64) #128 64

        # self.Up2 = up_conv(ch_in=128, ch_out=64) #128 64
        self.Att2 = Attention_block(F_g=32, F_l=32, F_int=16) #64 64 32
        self.Up_RRCNN2 = RRCNN_block(ch_in=64, ch_out=32, t=t) #128 64
...
#             nn.BatchNorm2d(64),
        ...
        # self.fcl1 = nn.Linear(200704, 1024)
        # self.fcl1 = nn.Linear(262144, 512)
        self.fcl1 = nn.Linear(9216, 2048)
        self.fcl2 = nn.Linear(2048, 512)
        self.fcl3 = nn.Linear(512, 64)
        self.fcl4 = nn.Linear(64, output_ch)
        # self.fcl1 = nn.Linear(4608, 512)
        # self.fcl1 = nn.Linear(4096, 512)
        ...
        # self.fcl6 = nn.Linear(8, 4)
        # self.fcl7 = nn.Linear(4, output_ch)
        self.fcl8 = nn.Linear(512, 32)
        self.fc20 = nn.Linear(512, 128)
        self.fcl9 = nn.Linear(200704, output_ch) #200704

        self.fc21 = nn.Linear(128, output_ch) #200704

    def forward(self, y): #y
        # encoding path
        # y1 = self.RRCNN1(y) #E-Res1-2
        y1 = self.RRCNN1_2(y) #E-Res1-1
        # y1 = self.conv1(y)
        # y2 = self.conv2(y1)
        # print("y1:",y1.shape)
        # x2 = self.Maxpool(x1) #E-Mp2-2
        # x2 = self.RRCNN2(x2) #E-Res2-2
        # y1 = self.dropout_0(y1)
        y2 = self.Maxpool(y1) #E-Mp2-1
        # print("y1:",y2.shape)
        y3 = self.RRCNN2(y2)  #E-Res2-1
        # print(y2.shape)
        # x3 = self.Maxpool(x2) #E-Mp3-2
        y4 = self.Maxpool(y3) #E-Mp3-1
        # #print(y3.shape,x3.shape)
        # # x3 = torch.cat((y2,x3),dim=1)
        # #print(x3.shape)
        
        y5 = self.RRCNN3(y4) #E-Res3-1
        # # print("x3-1:{}".format(x3.shape))
        # # x3 = self.Maxpool(x3)
        # # print("x3-2:{}".format(x3.shape))

        # d3 = self.Up3(x3) #D-UC2-1
        # # print("d3:{}".format(d3.shape))
        # # print("y2:{}".format(y2.shape))
        # y2 = self.Att3(g=d3, x=y2)
        # d3 = torch.cat((y2, d3), dim=1)
        # d3 = self.Up_RRCNN3(d3) #D-Res2-1
        # # print("d3:",d3.shape)

        # print("y3:",y3.shape)
        d3 = self.Up3(y5) #D-UC1-1
        # print("d3:",d3.shape)
        # print("y3:",y3.shape)

        y3 = self.Att3(g=d3, x=y3)
        # print("y1:",y1.shape)
        d3 = torch.cat((y3, d3), dim=1)
        # print("d2:",d2.shape)
        d3 = self.Up_RRCNN3(d3) #D-Res1-1
        # print("d2:",d2.shape)
        # d1 = self.Conv_1x1(d2)
        ...
        y1 = self.Att2(g=d2, x=y1)
        # print("y1:",y1.shape)
        d2 = torch.cat((y1, d2), dim=1)
        # print("d2:",d2.shape)
        d2 = self.Up_RRCNN2(d2) #D-Res1-1
