...
class Solver(object):
	def __init__(self, config, train_loader, valid_loader, test_loader):
		...
		print("img1_ch:{}".format(self.img1_ch))
	...
	def train(self):
		...
		if os.path.isfile(unet_path) and False:
			...
			if self.model_type =='U_Net':
				...
				model = R2U_Net(img_ch=3,output_ch=1,t=self.t)
			elif self.model_type =='AttU_Net':
				model = AttU_Net(img_ch=3,output_ch=self.output_ch)
			elif self.model_type == 'R2AttU_Net':
				model = R2AttU_Net(img_ch=self.img_ch,output_ch=self.output_ch,t=self.t)
				# device_ids = [i for i in range(int(torch.cuda.device_count()))]
				# print(torch.cuda.device_count())
				model = torch.nn.DataParallel(model.cuda())
			elif self.model_type == 'HSU_Net':
				model = HSU_Net(img1_ch=self.img1_ch,img2_ch=self.img2_ch, output_ch=self.output_ch, t=self.t)
				if torch.cuda.is_available():
					print("using cuda!")
					# self.unet = self.unet.cuda('cuda:1') #ProtoNet
					# print(next(self.unet.parameters()).device)
			...
			for epoch in range(self.num_epochs):
				...
				for i, data in enumerate(self.train_loader):
					...
					PR_all.extend(NR.cpu().argmax(dim=1))
					lable_all.extend(GT.cpu())
					# print(PR_all)
					# print(lable_all)
				...
				PR_all = np.array(PR_all).astype(np.int)
				lable_all = np.array(lable_all).astype(np.int)
				...
				with torch.no_grad():
					for i, data in enumerate(self.valid_loader):
						...
						SR = model(images2) #images2
						# SR = self.unet(images1,images2)
						acc += get_accuracy(SR,GT)

						PR_all.extend(SR.cpu().argmax(dim=1))
						lable_all.extend(GT.cpu())
				...
				# self.train_loader.shuffel_data()
			...
			with torch.no_grad():
				...
				print('[test] Acc: %.4f, OA: %.4f, recall: %.4f, precision: %.4f, F1: %.4f'%(acc,oa,recall,precision,F1))
