...
class ImageFolder(data.Dataset):
	...
	def __init__(self, root,image_size=224,mode='train',augmentation_prob=0.4):
		...
		self.mode = mode
		self.RotationuDegree = [0,90,180,270]
	...
	'''
	def __init__(self,image1,labels):
		self.image1 = image1
		self.labels = labels
		print(f"len:{len(self.image1)}")
		print(torch.tensor(self.image1).shape)
		#self.image,self.labels = MNISTreader.load_mnist()
	...
	def shuffel_data(self):
		print("shuffel dataset begin!")
		indexs = np.arange(self.labels.shape[0])
		np.random.shuffel(indexs)

		self.image1 = self.image1[indexs]
		self.labels = self.labels[indexs]
		print("shuffel dataset end!")

def get_loader(batch_size, num_workers=2,image1=None,labels=None):
	"""Builds and returns Dataloader."""
	#print("get_loader:",batch_size)
	dataset = ImageFolder(image1,labels)
