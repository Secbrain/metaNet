...
class Solver(object):
	...
	def train(self):
		...
		#====================================== Training ===========================================#
		#===========================================================================================#
		unet_path = os.path.join(cfg.model_save_path,f"R2AttUnet_{self.batch_size}_{self.num_epochs}_{self.lr}_{self.num_epochs_decay}.pkl")
		# U-Net Train
		if os.path.isfile(unet_path) and False:
			...
			for epoch in range(self.num_epochs):
				...
				for i, data in enumerate(self.train_loader):
					...
					PR_all = np.append(PR_all, NR.argmax(dim=1))
					lable_all = np.append(lable_all, GT)
				...
				for i, (images, GT) in enumerate(self.valid_loader):
					...
					PR_all = np.append(PR_all, SR.argmax(dim=1))
					lable_all = np.append(lable_all, GT)
					#SE += get_sensitivity(SR,GT)
					#SP += get_specificity(SR,GT)
					#PC += get_precision(SR,GT)
			...
			with torch.no_grad():
				...
				wr = csv.writer(f)
				print("test:",acc.item())
				wr.writerow([self.model_type,acc.item(),self.batch_size,self.lr,self.best_epoch,self.num_epochs,self.num_epochs_decay,self.augmentation_prob])
				f.close()
